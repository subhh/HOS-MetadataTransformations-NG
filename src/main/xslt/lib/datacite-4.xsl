<xsl:transform version="3.0" expand-text="yes"
  ...
  <xsl:template match="datacite-4:resource">
  ...
  <!-- 1. Identifikatoren -->
  <xsl:template match="datacite-4:identifier">
  ...
  <!-- 2. Titel -->
  <xsl:template match="datacite-4:title[not(@titleType)]">
    <dc:title>
      <xsl:sequence select="@xml:lang"/>
      <xsl:value-of select="normalize-space()"/>
      <xsl:if test="../datacite-4:title[@titleType = 'Subtitle']">
        <xsl:value-of select="' : ' || ../datacite-4:title[@titleType = 'Subtitle']"/>
      </xsl:if>
    </dc:title>
  </xsl:template>

  <!-- 3. Verfasser -->
  <xsl:template match="datacite-4:creator">
    <dct:creator>
      <dct:Agent>
        <skos:prefLabel>
  ...
  <!-- 4. Datum -->
  <xsl:template match="datacite-4:publicationYear">
    <dc:date>
      <xsl:value-of select="normalize-space()"/>
    </dc:date>
  </xsl:template>
  ...
  <!-- 5. Publisher -->
  <xsl:template match="datacite-4:publisher">
    <dc:publisher>
      <xsl:sequence select="@xml:lang"/>
      <xsl:value-of select="normalize-space()"/>
    </dc:publisher>
  ...
  <!-- 7. Sprache -->
  <xsl:template match="datacite-4:language">
    <dc:language>{library:normalize-language(normalize-space(.))}</dc:language>
  </xsl:template>

  <!-- 8. Schlagwörter -->
  <xsl:template match="datacite-4:subject">
    <dc:subject>
      <xsl:sequence select="@xml:lang"/>
      <xsl:value-of select="normalize-space()"/>
    </dc:subject>
  ...
  <!-- 9. Rechteangaben -->
  <xsl:template match="datacite-4:rights">
    <dc:rights>
      <xsl:sequence select="@xml:lang"/>
  ...
  <!-- 10. Beschreibungen -->
  <xsl:template match="datacite-4:description[@descriptionType = 'Abstract']">
    <dc:description>
      <xsl:sequence select="@xml:lang"/>
      <xsl:value-of select="normalize-space()"/>
    </dc:description>
  ...
  <xsl:template match="datacite-4:description[@descriptionType = 'Other']">
    <dc:description>
      <xsl:sequence select="@xml:lang"/>
      <xsl:value-of select="normalize-space()"/>
    </dc:description>
  ...
  <!-- 12. Beiträger -->
  <xsl:template match="datacite-4:contributor">
    <dct:contributor>
      <dct:Agent>
        <skos:prefLabel>
